...
export interface ToolsDateCalendarSolar2LunarModel {
  Animal: string;
  IDayCn: string;
  IMonthCn: string;
  Term: string | null;
...
export class ToolsDateCalendarService {
  /**
   * 农历1900-2100的润大小信息表
   * @Array Of Property
   * @return Hex
   ...
   */
  solar2lunar(y: number, m: any, d: number): ToolsDateCalendarSolar2LunarModel | number { // 参数区间1900.1.31~2100.12.31
    ...
    // 是否今天
    const isTodayObj = new Date(); let isToday = false
    if (isTodayObj.getFullYear() == y && isTodayObj.getMonth() + 1 == m && isTodayObj.getDate() == d) {
      isToday = true
    }
    // 是否可选 今天以前
    let isDisable = false
    if (isTodayObj.getTime() > new Date(`${y}/${m}/${d}`).getTime()) {
      isDisable = true
    }
    // 星期几
    let nWeek = objDate.getDay(); const cWeek = this.nStr1[nWeek]
    ...
    const firstNode = this.getTerm(year, (m * 2 - 1))// 返回当月「节」为几日开始
    const secondNode = this.getTerm(year, (m * 2))// 返回当月「节」为几日开始
    // 依据12节气修正干支月
    ...
    return {
      ...
      'astro': astro,
      'isDisable': isDisable,
      'lDateTimer': new Date(`${year}/${month}/${day}`).getTime(),
      'cDateTimer': new Date(`${y}/${m}/${d}`).getTime()
    }
  }
  /**
   ...
   */
  lunar2solar(y: number, m: any, d: number, isLeapMonth: boolean) { // 参数区间1900.1.31~2100.12.1
