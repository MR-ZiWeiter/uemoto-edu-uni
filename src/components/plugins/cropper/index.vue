...
export default {
  ...
  methods: {
            ...
            success: r => {
              ...
              if (!this.hasSel) {
                ...
                if (this.selWidth && this.selHeight) {
                  ...
                  const selHeight = this.selHeight.indexOf('rpx') >= 0 ? parseInt(this.selHeight) * this.pxRatio
    ...
    fDrawInit(ini = false) {
      const allWidth = this.windowWidth
      const allHeight = this.windowHeight
      const imgWidth = this.imgWidth
      const imgHeight = this.imgHeight
      const imgRadio = imgWidth / imgHeight
      let useWidth = allWidth
      let useHeight = allHeight - tabHeight
      const pixelRatio = this.pixelRatio
      const selWidth = parseInt(this.selStyle.width)
      const selHeight = parseInt(this.selStyle.height)
      ...
      const ctxCanvas = this.ctxCanvas
      const ctxCanvasOper = this.ctxCanvasOper
    ...
    async fEnd(e) {
      ...
      if (self.scaleSize < 1) {
        ...
        const firstWidth = this.postFirst.left - this.postFirst.posWidth
        const endHeight = this.posHeight - this.postFirst.posHeight
        const firstHigth = this.postFirst.top - this.postFirst.posHeight
